  ...
  <div class="card-content">
    ...
    $con=mysqli_connect("localhost","root","password","shoplada");
     ...
     $year = $_GET['year'];
      ...
      SUM((((p.price-p.discount)*s.amount) - pro.discountorder)) AS subtotal, COUNT(o.orderid) AS count
      FROM customer c, customerorder o, subcustomerorder s, voucher v,product p, promotion pro
      WHERE s.status = '3'
      AND DATE_FORMAT(s.`statustime`, '%m/%Y') = '$month/$year'
      AND c.userid = o.userid
      AND o.orderid = s.orderid
      AND o.voucherid = v.voucherid
      AND v.promoid = pro.promoid
      AND s.productid = p.productid
      AND s.version = p.version
      GROUP BY c.userid
      ORDER BY subtotal DESC, count DESC";
      $result = mysqli_query($con,$sql);
