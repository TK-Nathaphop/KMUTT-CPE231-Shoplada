  ...
  <div class="card-content">
    ...
    $con=mysqli_connect("localhost","root","password","shoplada");
     ...
     if(mysqli_connect_errno())
     {
      ...
      $sql = "SELECT s.shopname AS shop, p.productname AS product, SUM((p.price-p.discount)*sod.amount)*7/100 AS comission
      FROM shop s, product p, subcustomerorder sod
      WHERE s.shopid = p.shopid AND p.productid = sod.productid AND p.version = sod.version AND sod.status = 3
      GROUP BY s.shopname, p.Productname
      ...
      if(mysqli_num_rows($result) != 0)
        {
            echo '<table>
                   ...
                   <tbody>';
              for($i=0; $i<$amount && $data=mysqli_fetch_row($result); $i++)
              {
                echo '<tr>
                  <td>'.$data[0].'</td>
                  <td>'.$data[1].'</td>
