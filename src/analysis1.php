...
<div class="container">
  ...
  <div class="card-content">
    ...
    $con=mysqli_connect("localhost","root","password","shoplada");
     ...
     $year = $_GET['year'];
      $sql = "SELECT s.shopname AS `Shop Name`, p.productname AS Product, avg(r.star) AS Star
             FROM shop s, product p, review r, subcustomerorder sod
             WHERE DATE_FORMAT(r.`datetime`, '%m/%Y') = '$month/$year' AND
             p.shopid = s.shopid AND p.productid = sod.productid AND sod.suborderid = r.suborderid
