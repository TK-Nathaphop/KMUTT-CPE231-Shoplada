...
<div class="container">
  ...
  <div class="card-content">
    ...
    $con=mysqli_connect("localhost","root","password","shoplada");
     ...
     $year = $_GET['year'];
      $sql = "SELECT DATE_FORMAT(sod.`statustime`, '%m') AS month,SUM(((p.price - p.discount)*sod.amount) *7/100) AS commission
      FROM shop s, product p, subcustomerorder sod
      WHERE s.shopid = p.shopid AND p.productid = sod.productid AND p.version = sod.version AND sod.status = 3 AND DATE_FORMAT(sod.`statustime`, '%Y') = '2019'
      GROUP BY month
