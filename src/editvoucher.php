...
<div class="container">
  ...
  $sql = "SELECT v.VoucherID, v.PromoID, v.CategoryID, p.promoname, c.categoryname
  FROM voucher v, promotion p, category c
  WHERE v.promoid = p.promoid AND v.categoryid = c.categoryid AND v.voucherid = '$voucherid'";
  $result = mysqli_query($con,$sql);
  if($voucher = mysqli_fetch_row($result))
    {
    echo '<form action="php/update_voucher.php" method="get">
    ...
    <div class="card-content">
        ...
        <div class="row">
          <div class="input-field col s12 m12 l12 xl12">
            <input name="voucherid" id="voucherid" type="text" class="validate" value = "'.$voucher[0].'"required/>
            <label for="voucherid">Voucher ID</label>
          </div>
        </div>';
      $sql = "SELECT promoid, promoname FROM promotion";
      $result = mysqli_query($con,$sql);
      echo '<div class = "row">
          <div class="input-field col s6 m6 l6 xl6">
            <select name="promoid">
              <optgroup label="Old Promotion">
                <option value="'.$voucher[1].'">'.$voucher[3].'</option>
              </optgroup>
              <optgroup label="New Promotion">';
      while ($promo = mysqli_fetch_row($result))
        {
        if($promo[0] != $voucher[1])
          echo '<option value="'.$promo[0].'">'.$promo[1] .'</option>';
        }
      echo '</optgroup>
            </select>
      ...
      echo '<div class = "row">
          <div class="input-field col s6 m6 l6 xl6">
            <select name="categoryid">
              <optgroup label="Old Category">
                <option value="'.$voucher[2].'">'.$voucher[4].'</option>
              </optgroup>
              <optgroup label="New Category">';
      while ($promo = mysqli_fetch_row($result))
        {
        if($promo[0] != $voucher[2])
          echo '<option value="'.$promo[0].'">'.$promo[1] .'</option>';
        }
      echo '</optgroup>
            </select>
    ...
    <p class="right-align">
    <a href="http://e22vvb.asuscomm.com:43221/php/delete_voucher.php?voucherid='.$voucherid.'" class="btn waves-light card-panel red">Delete Voucher</a>
    <button class="btn waves-light card-panel green" type="submit" name="action" value ="'.$voucherid.'">Update</button>
    </p>
    </form>';
    }
  else
    echo "<script type='text/javascript'>alert('Cannot find the voucher');location= 'http://e22vvb.asuscomm.com:43221/promotion.php';</script>";
  ?>
</div>
