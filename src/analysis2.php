...
<div class="container">
  ...
  <div class="card-content">
    ...
    $con=mysqli_connect("localhost","root","password","shoplada");
     ...
     $year = $_GET['year'];
      $sql = "SELECT s.shopname AS `Shop Name`, p.productname AS Product, sum(sod.amount) AS Amount
             FROM shop s, product p, subcustomerorder sod
             WHERE DATE_FORMAT(sod.`statustime`, '%m/%Y') = '$month/$year' AND sod.status = '3' AND
             p.shopid = s.shopid AND p.productid = sod.productid
             GROUP BY `Shop Name`, Product
