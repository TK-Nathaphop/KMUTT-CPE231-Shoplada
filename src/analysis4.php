...
<div class="container">
  ...
  <div class="card-content">
    ...
    $con=mysqli_connect("localhost","root","password","shoplada");
     ...
     $year = $_GET['year'];
      $sql = "SELECT cat.categoryname AS `Category`, sum(sod.amount) AS Amount
             FROM category cat, subcategory scat, subcustomerorder sod, product p
             WHERE DATE_FORMAT(sod.`statustime`, '%m/%Y') = '$month/$year' AND
             sod.status = '3' AND
             cat.CategoryID = scat.CategoryID AND
             scat.SubCategoryID = p.SubCategoryID AND
